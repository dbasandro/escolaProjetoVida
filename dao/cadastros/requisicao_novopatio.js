...
module.exports = {
    pageRequisicao_novopatio: (req, res) => {
        ...
        (async function () {
            ...
            page = './includes/cadastros/inc_requisicao_novopatio';

         
            db.query("CALL sp_consulta_produto(?,?)", [3, cod_user], 
            function (err, result) {
                //console.log(result[0])
                
                     //PERMISSOES COMUNICAÇÃO 
                     let Comunicacaosubmenu = "select m.nome as nome, pagina, icone from sub_menu m inner join selecao s on s.cod=m.id_selecao inner join permissao p on s.cod=p.id_selecao inner join tb_usuario u on u.cod=p.id_usuario and u.cod in (select cod from tb_usuario where nome = '" + req.session.nome_login +"') and m.id_menu=1 and p.status=1 and p.id_unidade=3";
                     let Comunicacaomenu= "select m.nome as nome from menu m inner join selecao s on s.cod=m.id_selecao inner join permissao p on s.cod=p.id_selecao inner join tb_usuario u on u.cod=p.id_usuario and u.cod in (select cod from tb_usuario where nome = '" + req.session.nome_login +"') and s.cod = 1 and p.status=1 and p.id_unidade=3";
                  ...
                  // db.query(cadastrosubmenu, (err, cadastrosubmenu_reposta, fields) => {
                   ...
                   db.query(Pedidomenu, (err, pedidomenu_resposta, fields) => {   
                     ...
                     //PERMISSOES ENTREGAS    
                     let Entregasubmenu = "select m.nome as nome, pagina, icone from sub_menu m inner join selecao s on s.cod=m.id_selecao inner join permissao p on s.cod=p.id_selecao inner join tb_usuario u on u.cod=p.id_usuario and u.cod in (select cod from tb_usuario where nome = '" + req.session.nome_login +"') and m.id_menu=22 and p.status=1 and p.id_unidade=3";
                     let Entregamenu= "select m.nome as nome from menu m inner join selecao s on s.cod=m.id_selecao inner join permissao p on s.cod=p.id_selecao inner join tb_usuario u on u.cod=p.id_usuario and u.cod in (select cod from tb_usuario where nome = '" + req.session.nome_login +"') and s.cod = 22 and p.status=1 and p.id_unidade=3";
                     ...
                     let Relatoriosubmenu = "select m.nome as nome, pagina, icone from sub_menu m inner join selecao s on s.cod=m.id_selecao inner join permissao p on s.cod=p.id_selecao inner join tb_usuario u on u.cod=p.id_usuario and u.cod in (select cod from tb_usuario where nome = '" + req.session.nome_login +"') and m.id_menu=3 and p.status=1 and p.id_unidade=3";
                     let Relatoriomenu= "select m.nome as nome from menu m inner join selecao s on s.cod=m.id_selecao inner join permissao p on s.cod=p.id_selecao inner join tb_usuario u on u.cod=p.id_usuario and u.cod in (select cod from tb_usuario where nome = '" + req.session.nome_login +"') and s.cod = 3 and p.status=1 and p.id_unidade=3";
    ...
    addRequisicao_novopatio: (req, res) => {
            ...
            observacao + "')";
                    ...
                    db.query(insertpedito, (err, resultadoinsertpedito, fields) => {
                        ...
                        console.log(resultadoinsertpedito)
                            ...
                            status_Crud = 'nao';
                            res.redirect('/requisicao_novopatio');
                        } else {
                  ...
                  db.query(sql, [pedido], function (error, results40, fields) {                      if (error) console.log(error)
                     ...
                     queryResults1 = results40[0]
                      ...
                      var emailASerEnviado = {
                        ...
                        to: 'Almoxarifado@novopatio.com.br',
...
conn.query(sql, [values], function(err) {
    ...
    editRequisicao_novopatio: (req, res) => {
    ...
    delRequisicao_novopatio: (req, res) => {

...
module.exports = {
    pageRequisicao_sandro: (req, res) => {
        ...
        (async function () {
            ...
            //let html = await DBModel.gethtml();


            let ConsultaTurma = "SELECT unidade,CONCAT( tt.serie, ' ',  tc.classe, ' ', tp.periodo, ' ', tt.anoletivo ) as turma, tt.cod FROM tb_turma  tt inner join tb_classe tc on tt.classe=tc.cod inner join tb_periodo tp on tt.periodo=tp.cod and tt.status=1 and unidade =2"
            db.query(ConsultaTurma, (err, Turmas2, fields) => {
            ...
            page = './includes/cadastros/inc_requisicao_sandro';

         
            db.query("CALL sp_consulta_produto(?,?)", [2, cod_user], 
            function (err, result) {
                //console.log(result[0])
                
                     //PERMISSOES COMUNICAÇÃO 
                     let Comunicacaosubmenu = "select m.nome as nome, pagina, icone from sub_menu m inner join selecao s on s.cod=m.id_selecao inner join permissao p on s.cod=p.id_selecao inner join tb_usuario u on u.cod=p.id_usuario and u.cod in (select cod from tb_usuario where nome = '" + req.session.nome_login +"') and m.id_menu=1 and p.status=1 and p.id_unidade=2";
                     let Comunicacaomenu= "select m.nome as nome from menu m inner join selecao s on s.cod=m.id_selecao inner join permissao p on s.cod=p.id_selecao inner join tb_usuario u on u.cod=p.id_usuario and u.cod in (select cod from tb_usuario where nome = '" + req.session.nome_login +"') and s.cod = 1 and p.status=1 and p.id_unidade=2";
                    ...
                    if (paginas == 2){
                         redireciona = '/material_sandro'
                         res_requisicao = '/requisicao_sandro'
                    }
                    if (paginas == 3){
                   ...
                   // db.query(cadastrosubmenu, (err, cadastrosubmenu_reposta, fields) => {
                    ...
                    db.query(Pedidomenu, (err, pedidomenu_resposta, fields) => {   
                     ...
                     //PERMISSOES ENTREGAS    
                     let Entregasubmenu = "select m.nome as nome, pagina, icone from sub_menu m inner join selecao s on s.cod=m.id_selecao inner join permissao p on s.cod=p.id_selecao inner join tb_usuario u on u.cod=p.id_usuario and u.cod in (select cod from tb_usuario where nome = '" + req.session.nome_login +"') and m.id_menu=22 and p.status=1 and p.id_unidade=2";
                     let Entregamenu= "select m.nome as nome from menu m inner join selecao s on s.cod=m.id_selecao inner join permissao p on s.cod=p.id_selecao inner join tb_usuario u on u.cod=p.id_usuario and u.cod in (select cod from tb_usuario where nome = '" + req.session.nome_login +"') and s.cod = 22 and p.status=1 and p.id_unidade=2";
                     ...
                     let Relatoriosubmenu = "select m.nome as nome, pagina, icone from sub_menu m inner join selecao s on s.cod=m.id_selecao inner join permissao p on s.cod=p.id_selecao inner join tb_usuario u on u.cod=p.id_usuario and u.cod in (select cod from tb_usuario where nome = '" + req.session.nome_login +"') and m.id_menu=3 and p.status=1 and p.id_unidade=2";
                     let Relatoriomenu= "select m.nome as nome from menu m inner join selecao s on s.cod=m.id_selecao inner join permissao p on s.cod=p.id_selecao inner join tb_usuario u on u.cod=p.id_usuario and u.cod in (select cod from tb_usuario where nome = '" + req.session.nome_login +"') and s.cod = 3 and p.status=1 and p.id_unidade=2";
            ...
            atualizabox = req.body.id_departamento_ADD
             ...
             if (atualizabox.substr(-1) == 0) {
                ...
                id_disciplina = 41
                    ...
                    db.query(insertpedito, (err, resultadoinsertpedito, fields) => {
                        ...
                        console.log(resultadoinsertpedito)
                            ...
                            status_Crud = 'nao';
                            res.redirect('/requisicao_sandro');
                        } else {
                ...
                function sqlQuery (db, pedido) {
                  ...
                  db.query(sql, [pedido], function (error, results40, fields) {                      if (error) console.log(error)
                     //db.end()

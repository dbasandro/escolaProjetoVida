...
module.exports = {
    pageEntrega_sandro: (req, res) => {
        ...
        (async function () {
            ...
            page = './includes/cadastros/inc_entrega_produto_sandro';
    ...
    console.log(id_pedido,cod_user, id_produto, id_entrega, cod_unidade)
            ...
            function (error, results41, fields) {
                if (error) {
                    status_Crud = 'nao';
                    res.redirect('/entrega_produto_sandro');
                }
                else {
    ...
    delEntrega_sandro: (req, res) => {
        console.log(req.body)
        let cod = req.params.id;
        let getImageQuery = 'SELECT foto FROM `tb_usuario` WHERE cod = "' + cod + '"';
        let query = 'DELETE FROM `tb_pedido_item` WHERE ID_PRODUTO = "' + cod + '" AND ID_PEDIDO = "' + cod + '"';

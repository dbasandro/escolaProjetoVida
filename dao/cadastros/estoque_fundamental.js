...
module.exports = {
    pageMaterial_fundamental: (req, res) => {
        ...
        (async function () {
         ...
         //  let familias = await DBModel.getFamiliaMaterial();
            ...
            page = './includes/cadastros/inc_material_fundamental';
       ...
       let resposta = "select (select status as baixa from permissao where id_usuario='" + cod_user + "' and id_unidade=2 and id_selecao=29) as baixa,(select status as produto from permissao where id_usuario='" + cod_user + "' and id_unidade=2 and id_selecao=30) as produto,  (select status as adicionar from permissao where id_usuario='" + cod_user + "' and id_unidade=2 and id_selecao=42) as adicionar,(select status as editar from permissao where id_usuario='" + cod_user + "' and id_unidade=2 and id_selecao=43) as editar,  (select status as eliminar from permissao where id_usuario='" + cod_user + "' and id_unidade=2 and id_selecao=44) as eliminar"
...
function (err, result) {
  ...
  permissoes = result[0]
    ...
    addMaterial_fundamental: (req, res) => {
...
//console.log (material, descricao )
            ...
            db.query(query, (err, results, fields) => {
                if (results.length > 0) {
                    ...
                    res.redirect('/material_fundamental');
                } else {
                    if (!req.files) {}
                    else{
                    ...
                    db.query("CALL sp_insert_produto(?,?,?,?,?)", [material, descricao, categoria,unidade,image_name], 
                        function (err, result) {
                            ...
                            db.query("CAll sp_auditoria(?,?,?)", [2, req.session.cod_login, conteudo], 
    ...
    editMaterial_fundamental: (req, res) => {
    ...
    editAdicionaMaterial_fundamental: (req, res) => {
    ...
    editBaixaMaterial_fundamental: (req, res) => {
    ...
    console.log(req.body)
        ...
        if (!req.files) {
            ...
            db.query("CALL atualiza_BAIXA_ESTOQUE(?,?,?,?,?,?)", [cod_usuario,saida,observacao,cod_material,2,classificacao], 
            function (err, result) {
                console.log(result)
                if (err) {
                    console.log('Erro 003: ', err);
    ...
    delMaterial_fundamental: (req, res) => {

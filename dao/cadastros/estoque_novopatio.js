...
module.exports = {
    pageMaterial_novopatio: (req, res) => {
        ...
        (async function () {
         ...
         //  let familias = await DBModel.getFamiliaMaterial();
            ...
            page = './includes/cadastros/inc_material_novo_patio';
       ...
       let resposta = "select (select status as baixa from permissao where id_usuario='" + cod_user + "' and id_unidade='" + 3 + "' and id_selecao=29) as baixa,(select status as produto from permissao where id_usuario='" + cod_user + "' and id_unidade='" + 3 + "' and id_selecao=30) as produto,  (select status as adicionar from permissao where id_usuario='" + cod_user + "' and id_unidade='" + 3 + "' and id_selecao=42) as adicionar,(select status as editar from permissao where id_usuario='" + cod_user + "' and id_unidade='" + 3 + "' and id_selecao=43) as editar,  (select status as eliminar from permissao where id_usuario='" + cod_user + "' and id_unidade='" + 3 + "' and id_selecao=44) as eliminar"
...
//console.log (material, descricao )
            ...
            db.query(query, (err, results, fields) => {
                if (results.length > 0) {
                    ...
                    res.redirect('/material_novopatio');
                } else {
                    if (!req.files) {}
                    else{
                    ...
                    db.query("CALL sp_insert_produto(?,?,?,?,?)", [material, descricao, categoria,unidade,image_name], 
                        function (err, result) {
                            ...
                            db.query("CAll sp_auditoria(?,?,?)", [3, req.session.cod_login, conteudo], 
    ...
    editMaterial_novopatio: (req, res) => {
    ...
    editAdicionaMaterial_novopatio: (req, res) => {
    ...
    editBaixaMaterial_novopatio: (req, res) => {
    ...
    console.log(req.body)
        ...
        if (!req.files) {
            ...
            db.query("CALL atualiza_BAIXA_ESTOQUE(?,?,?,?,?,?)", [cod_usuario,saida,observacao,cod_material,3,classificacao], 
            function (err, result) {
                if (err) {
                    console.log('Erro 003: ', err);
                    status_Crud = 'nao';
